...
' ============================================================
' modPartsIndex - Parts Database インデックス管理モジュール
' ============================================================
' ★右1マスずらし版★
'   入力シート側の列:
'     発火トリガ: H列（元G列）
'     転記先:     F列←DB A列, G列←DB B列, H列←DB C列（元E/F/G列）
'     一致判定:   D列（元C列）
'     赤背景:     K列（元J列）
'   parts database 側の列: 変更なし（A/B/C/D/E列）
' ============================================================

' --- 定数 ---
' ネットワークパスを設定する場合はここを変更
' 例: "\\server\share\folder\parts database.xlsm"
' 空文字の場合、ブックAと同じフォルダから検索する
...
' --- 入力シート側の列定数（右1マスずらし） ---
Public Const COL_TRIGGER As String = "H"       ' 発火トリガ列（元G列）
Public Const COL_TRIGGER_NUM As Long = 8        ' H列 = 8
Public Const COL_TRANSFER_A As String = "F"     ' DB A列の転記先（元E列）
Public Const COL_TRANSFER_B As String = "G"     ' DB B列の転記先（元F列）
Public Const COL_TRANSFER_C As String = "H"     ' DB C列の転記先（元G列）
Public Const COL_MATCH As String = "D"          ' 一致判定列（元C列）
Public Const COL_RED_BG As String = "K"         ' 赤背景列（元J列）
...
' --- フォーム連携用グローバル ---
Public g_InitialKey As String
Public g_SelectedIndex As Long          ' -1 = キャンセル, >0 = 選択されたインデックス
...
Public Sub BuildPartsIndex()
    ...
    ' 全シートを走査してインデックス構築（parts database の列は変更なし）
...
Public Function SearchCandidates(ByVal key As String) As Long
    ...
    For i = 1 To g_IndexCount
        ...
        If MatchPos > 0 Then
            ...
            LenDiff = Abs(Len(normalizedC) - Len(normalizedKey))
...
Public Sub TransferData(ByVal targetRow As Long, ByVal candidateIdx As Long)
    ...
    ' --- 対象行に転記 ---
    ' DB A列 → 入力シート F列（元E列）
    ws.Cells(targetRow, COL_TRANSFER_A).Value = rec.ColA
    ' DB B列 → 入力シート G列（元F列）
    ws.Cells(targetRow, COL_TRANSFER_B).Value = rec.ColB
    ' DB C列 → 入力シート H列（元G列）
    ws.Cells(targetRow, COL_TRANSFER_C).Value = rec.ColC
    ' K列背景色（元J列）
    ApplyRedBackground ws, targetRow, rec.ColE

    ' --- D列一致行への一括反映（元C列） ---
    cVal = rec.ColC
    lastRow = ws.Cells(ws.Rows.Count, COL_MATCH).End(xlUp).Row

    If lastRow >= 2 Then
        ' D列を配列で一括読み込み（高速化）
        matchRange = ws.Range(COL_MATCH & "2:" & COL_MATCH & lastRow).Value

        For r = 1 To UBound(matchRange, 1)
            If (r + 1) <> targetRow Then
                If CStr(SafeCStrVariant(matchRange(r, 1))) = cVal Then
                    ws.Cells(r + 1, COL_TRANSFER_A).Value = rec.ColA
                    ws.Cells(r + 1, COL_TRANSFER_B).Value = rec.ColB
                    ws.Cells(r + 1, COL_TRANSFER_C).Value = rec.ColC
                    ApplyRedBackground ws, r + 1, rec.ColE
                End If
            End If
        Next r
    End If
...
' ============================================================
' Variant のセーフ文字列変換（配列読み込み用）
' ============================================================
Private Function SafeCStrVariant(ByVal v As Variant) As String
    If IsError(v) Then
        SafeCStrVariant = ""
    ElseIf IsNull(v) Or IsEmpty(v) Then
        SafeCStrVariant = ""
    Else
        SafeCStrVariant = CStr(v)
    End If
End Function
...
' ============================================================
Public Sub ProcessColumnInput(ByVal targetRow As Long, ByVal key As String)
    Dim totalCount As Long
