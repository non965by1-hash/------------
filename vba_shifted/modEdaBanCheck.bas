...
' ============================================================
' modEdaBanCheck - 枝番チェック マクロ（右1マスずらし版）
' ============================================================
...
'   5. total = vCountSum + aBonus >= 2 なら、そのI値の全行のK列を薄いブルーに塗る
'      ただし既にK列に背景色があるセルは変更しない
' ============================================================
...
' --- 着色対象列（右1マスずらし版：J→K） ---
Private Const COL_EDABAN_BG As String = "K"
    ...
    For Each iKey In dictIRows.Keys
        ...
        If total >= 2 Then
            ...
            For idx = 1 To rowCol.Count
                ...
                rn = rowCol(idx)
                ' 既にK列に背景色がある場合は変更しない
                If ws.Cells(rn, COL_EDABAN_BG).Interior.Pattern = xlNone Then
                    ws.Cells(rn, COL_EDABAN_BG).Interior.Color = lightBlue
                End If
            Next idx
        End If
    Next iKey
